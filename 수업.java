...
public class 수업 {
	...
	public static void main(String[] args) {
		...
		// ArrayList 컬렉션
		ArrayList<Integer> list = new ArrayList<Integer>(); // 컨트롤 + 쉬프트 + o 임포트 하기
		/*
		 * 제네릭(<>) 특징
		 * 1. 클래스 타입만 넣을 수 있음, int 형을 담고 싶다면 int에 클래스 타입인 Integer를 작성
		 */
		...
		System.out.println("0번째 조회 : " + list.get(0)); // 100!
		...
		System.out.println("list에 평균 : " + sum / list.size());
		
		// HashSet 컬렉션즈
		/*
		 * HashSet : 중복 제거가 필요하거나 집합 연산을 수행해야 할 때 유용하게 사용함
		 * 1. 중복을 허용하지 않는 고유한 요소만 저장
		 */
		HashSet<Integer> set = new HashSet<Integer>(); // 임포트
		set.add(10); // 0번째 요소에 10추가 
		set.add(20); // 1번째 요소에 20추가 
		set.add(10); // 2번째 요소에 10추가 -> 중복된 값을 무시함
		...
		System.out.println("HashSet 크기 : " + set.toString());
		System.out.println("HashSet 크기 : " + set.size());
